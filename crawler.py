...
class PttWebCrawler(object):
                ...
                for div in divs:
                    try:
                        ...
                        if div == divs[-1] and i == end-start:  # last div of last page
                            self.store(filename, parse(link, article_id, board), 'a')
                        else:
                            self.store(filename, parse(link, article_id, board) + ',\n', 'a')
                    except:
                        pass
                time.sleep(0.1)
    ...
    def parse_article(self, article_id, board, path='.'):
        ...
        filename = os.path.join(path, filename)
        self.store(filename, parse(link, article_id, board), 'w')
        return filename
...
def parse(link, article_id, board, timeout=3):
    ...
    Total = {'all': p+b+n, 'count': p-b, 'push': p, 'boo': b, "neutral": n}
